...
import { kvGetPageViews, KV_AVAILABLE } from "@/lib/analytics/kv";
  ...
  return (
    <div className="flex flex-col gap-10">
      <div>
        ...
        <p className="text-text-subtle font-body text-sm">Page views tracked by BNSignal</p>
      </div>
      ...
      {!KV_AVAILABLE && (
        <div className="rounded-lg border border-gold/30 bg-gold/5 p-4">
          ...
          <p className="font-body text-xs text-text-muted">
            Create a KV database in your Vercel dashboard → Storage tab, then connect it to this project. Env vars are added automatically.
          </p>
        </div>
      )}
          ...
          <div className="flex items-end gap-0.5 h-20">
            {daily.map(([date, count]) => (
              <div key={date} className="flex-1 flex flex-col items-center group relative">
                <span className="absolute -top-5 left-1/2 -translate-x-1/2 text-[9px] font-body text-gold opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">
                  {count}
                </span>
                <div
        ...
        <div className="rounded-lg border border-white/10 bg-surface p-8 text-center">
          <p className="text-text-subtle font-body text-sm">
            {KV_AVAILABLE
              ...
              : "Connect a Vercel KV store to start tracking page views."}
          </p>
        </div>
      )}
    </div>
